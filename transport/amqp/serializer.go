...
func NewSerializer(resolver transport.TypeResolver) *Serializer {
	return &Serializer{resolver: resolver}
}

func (s *Serializer) Marshal(env *envelope.Envelope) ([]byte, map[string]string, error) {
	msg := env.Message()
...
func (s *Serializer) Unmarshal(body []byte, headers map[string]string) (*envelope.Envelope, error) {
	...
	if rawStamps, ok := headers["stamps"]; ok {
		...
		for _, sStamp := range sStamps {
			...
			env = env.WithStamp(ptr.(envelope.Stamp))
		}
	}
